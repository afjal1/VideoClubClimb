...
class UploadVideoMenuBloc
    extends Bloc<UploadVideoMenuEvent, UploadVideoMenuState> {
  DataRepo dataRepo;

  UploadVideoMenuBloc({required this.dataRepo})
      : super(
            UploadVideoMenuState(categories: [], videos: [], videosNames: [])) {
    on<CategoryClickedUploadVideoMenuEvent>(
        _getVideosInACategoryUploadVideoMenu);
    on<GetCategoriesUploadVideoMenuEvent>(_getCategoriesUploadVideoMenu);
  }

  _getCategoriesUploadVideoMenu(GetCategoriesUploadVideoMenuEvent event,
      Emitter<UploadVideoMenuState> emit) async {
    emit(UploadVideoMenuFormSubmitting(
        categories: [], videos: [], videosNames: []));
    await dataRepo.listItems();

    List<String> categories = await dataRepo.getCategoriesSectoresMenu();
    emit(state.copyWith(
        totalCategories: categories.length, categories: categories));
  }

  FutureOr<void> _getVideosInACategoryUploadVideoMenu(
      CategoryClickedUploadVideoMenuEvent event,
      Emitter<UploadVideoMenuState> emit) {
  ...
  FutureOr<void> _refreshListsSubirVideoMenu(
      RefreshListsSubirVideoMenuEvent event,
      Emitter<UploadVideoMenuState> emit) async {
  ...
  FutureOr<void> _searchVideosSubirVideoMenu(
      SearchEventSubirVideoMenu event, Emitter<UploadVideoMenuState> emit) {
